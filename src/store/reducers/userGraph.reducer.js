...
export default usergraphReducer = (state = initialState, action) => {
    // console.log('usergraphReducer', action);
    switch (action.type) {
        case GET_USER_DATA:
            return { ...state, isLoadingData: true, showErrorUser: false, successDataUser: false };

        case GET_USER_DATA_SUCCESS:
            console.log('user info for pipe', action.payload)
            return {
                ...state,
                isLoadingData: false,
                showErrorUser: false,
                ...
                dataUser: {
                    ...action.payload,
                    clients: {
                        ...action.payload.clients[0]
                    },
                    usersProfile: {
                        ...action.payload.usersProfile[0],

                        accounts: {
                            ...action.payload.usersProfile[0].accounts,
                            // address: {
                            //     ...action.payload.usersProfile[0].accounts.address[0]
                            // },
                            kyc: {
                                ...action.payload.usersProfile[0].accounts.kyc[0]
                            }
                        }
                    }
                },
