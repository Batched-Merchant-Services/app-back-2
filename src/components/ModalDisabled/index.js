...
import { getAccountsParameter } from '../../utils/api/graph_api/utils.service';

const ModalDisabled = ({ isOpen, navigation, onClose = () => null, }) => {
    const redux = useSelector(state => state);
    const appData = redux.user;
    const brandTheme = appData?.Theme?.colors;
    var [urlBank, setUrlBank] = useState('');
    ...
    async function getUrlCardportal() {
        let response = await getAccountsParameter('bank-prod');
        console.log('url for bank', response);
        setUrlBank(response?.getAccountsParameter);
    }
        ...
        <ModalContainer showModal={!showOpen ? showOpen : Open}>
            <View centerH style={styles.containerModal}>
                ...
                <View paddingH-20>
                    ...
                    <DivSpace height-28 />
                    <Text h11 style={styles.textBgblue} center>
                        {i18n.t('ModalDisabled.component.textIfYouWantMore')} {urlBank}
                    </Text>
                </View>
                <DivSpace height-40 />
                <View row>
