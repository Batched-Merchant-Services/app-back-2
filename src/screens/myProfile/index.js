...
import { cleanDataFile, setFile } from '../../store/actions/userGraph.actions';
...
function selectAvatar(setShowPhoto, setShowImage, setIsLoadingModal,dispatch) {
...
const myProfile = ({ navigation }) => {
  ...
  const [id, setId] = useState(userAccount?.id);
  ...
  function getImageProfile() {
    setShowPhoto({ uri: userAccount?.avatarImage + '?' + new Date() });
    if (userGraph?.setFile !== '')  setShowImage(true);
  ...
  const handleChoosePhoto = async () => {
    selectAvatar(setShowPhoto, setShowImage, setIsLoadingModal,dispatch);
  };
