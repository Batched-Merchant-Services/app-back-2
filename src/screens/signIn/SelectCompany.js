...
import { getCompaniesGraph } from '../../utils/api/graph';
...
async function handleSignIn(
    ...
    if (response?.getLoggin) {
        ...
        if (isToken === '' && deviceStatus === 100 || deviceStatus === 303) {
            ...
            console.log('device active', deviceStatus)
...
const SelectCompany = ({ navigation, loginWithFingerPrint, toggleLoginWithFingerprint, screenProps }) => {
    ...
    async function getCompanies() {
        setIsLoadingModal(true);
        const response = await getCompaniesGraph(phone);
        // const response = await getCompaniesQuery(phone);

        let companies = response?.getUserCompanies;

        if (companies) {
            let values = [];
            companies.forEach((element, idx) => values.push({
                id: idx + 1,
